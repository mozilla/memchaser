...
exports.test_state = function(test) {
  ...
  test.assert(!buffer.isEmpty(), "Buffer should not be empty");
  test.assert(!buffer.isFull(), "Buffer should not be full");
...
exports.test_readwrite = function(test) {
  ...
  test.assert(buffer.isEmpty(), "Buffer should be empty after a read without content");

  buffer.write({resident:1,GC:2,CC:3});
  test.assertEqual(buffer.read().GC, 2, "GC field in retrieved data should be 2");
  test.assert(buffer.isEmpty(), "Buffer should technically be empty after a reading all data");
  ...
  test.assertEqual(buffer.read().GC, 5, "GC field in retrieved data should be 5");
  test.assertEqual(buffer.read().GC, 6, "GC field in retrieved data should be 6");
  test.assertEqual(buffer.read().GC, 7, "GC field in retrieved data should be 7");
  test.assertEqual(buffer.read().GC, 8, "GC field in retrieved data should be 8");
  test.assert(buffer.isEmpty(), "Buffer should technically be empty after a reading all data");
};
...
exports.test_clear = function(test) {
  ...
  buffer.write({resident:1,GC:6,CC:3});
  buffer.clear();
  test.assert(buffer.isEmpty(), "Buffer should be empty after a clear");
};